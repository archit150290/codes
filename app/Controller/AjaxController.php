...
//                ))
        ...
        if ($orgType == 'all' || $orgType == 'inactive') {
            $joinType = 'LEFT';
        } else {
            $joinType = 'RIGHT';
        }
...
//        header("location:".$tmpFile);   
    ...
    public function downloadallendorsementlist(){
        $this->autoRender = false;
        $this->layout = "ajax";
        $alldata = json_decode($this->request->data["alldata"], true);
        $tableheader = json_decode($this->request->data["tableheader"], true);
        $totaler = $this->request->data["totaler"];
        $endorsementreceived = array();
        $endorsementgiven = array();
        //==========collecting endorsement received
        for($i = 0; $i < $totaler; $i++){
           array_push($endorsementreceived, $alldata[$i]);
        }
        //==========collecting endorsement given
        for($i = $totaler; $i < count($alldata); $i++){
           array_push($endorsementgiven, $alldata[$i]);
        }
