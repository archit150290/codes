...
class AnalyticsController extends AppController {
     ...
     */
    public $uses = array();
     private $siteId = 'f7438d51cd8064e786e442230c76debc'; 
    private $apiKey = 'd768fd7a6087bcd64148c65368f9f6e0';
    ...
    public function beforeFilter() {
        ...
        if ($_SERVER["HTTP_HOST"] == "localhost") {
            $this->siteId = '513cb76b8662344716527f2cd6583625';
            $this->apiKey = 'e173b2c33074765e7ea5942eda16e67a';
    ...
    function index() {
        ...
        $response = json_decode($this->AmpushApi->callUrl($urlOffset, $query), true);
        $this->layout = "tabular";

        $visitorTable = array("title" => "Visitor Details", "cols" => array("visitor_id", "session_last_req", "session_num_pageviews", "location_country", "location_city", "ip_address", "browser_type", "os", "referer_url", "medium", "document_url"));
        if (isset($response["resultsRows"])) {
            $visitorTable["data"] = $response["resultsRows"];
        }
        $tablesData = array("visitor_details" => $visitorTable);
        $visitorTable = null;
        //pr($tablesData);
    ...
    function _getPlanTable($visitorId = '') {
        ...
        if (isset($response["resultsRows"])) {
            ...
            foreach ($response["resultsRows"] as $i => $item) {
                ...
                if(!isset($ratingItems[1])) continue;
        ...
        ksort($data["rows"]);
        $data["common"]["box_size"] = isset($data["common"]["box_size"]) && is_array($data["common"]["box_size"]) ?  array_flip($data["common"]["box_size"]) : array();
        $data["common"]["delivery_frequency"] = isset($data["common"]["delivery_frequency"]) && is_array($data["common"]["delivery_frequency"]) ? array_flip($data["common"]["delivery_frequency"]) : array();
        ...
        if (is_array($data["common"]["delivery_frequency"])) {
            ...
            $data["common"]["delivery_frequency"] = isset($mapping[$fre]) ? $mapping[$fre] : "";
        }
