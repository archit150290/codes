        ...
        if (formId == "new_user") {
            var setIntervalform = setInterval(function () {
                if (uniqueness_checked == 1 && objValidate.status == true && emailPerfect == true) {
                    ...
                    console.log('email-loop-out-FB');
                    $("#" + formId + " .email").show();
                    $("#" + formId + " #user_email").attr({
                        type: 'email'
                    });
                    clearInterval(setIntervalform);
                }
...
$(document).ready(function(){
    $("#new_user_pop input[type='radio'].rbtn-scale-pop, #new_user input[type='radio'].rbtn-scale").attr('checked', false);
});
