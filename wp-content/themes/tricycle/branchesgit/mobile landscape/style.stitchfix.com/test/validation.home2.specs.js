describe("Unit Testing Cases", function () {
    ...
    describe("Validate Functionality for form New User Auto Pop Up Form", function () {
        //======cases for pop up form

        it('Test 22 Case When Only First Name is there and removed', function () {
            $("#new_user_auto_pop #user_first_name").val("dugar")
            $("#new_user_auto_pop #user_first_name").blur();
            expect(objValidateAutoPop.items["user_first_name"].status).to.be.true
            $("#new_user_auto_pop #user_first_name").val("")
            $("#new_user_auto_pop #user_first_name").blur();
            expect(objValidateAutoPop.items["user_first_name"].status).to.be.false;
        });

        it('Test 23 Case When Only Last Name is there and removed', function () {
            console.log('Test 24')
            $("#new_user_auto_pop #user_last_name").val("dugar")
            $("#new_user_auto_pop #user_last_name").blur();
            expect(objValidateAutoPop.items["user_last_name"].status).to.be.true
            $("#new_user_auto_pop #user_last_name").val("")
            $("#new_user_auto_pop #user_last_name").blur();
            expect(objValidateAutoPop.items["user_last_name"].status).to.be.false;
        });

        it('Test 24 Case When Only Email is there and removed', function () {
            console.log('Test 24')
            $("#new_user_auto_pop #user_email").val("dugararchit@gmail.com");
            $("#new_user_auto_pop #user_email").blur();
            expect(objValidateAutoPop.items["user_email"].status).to.be.true;
            $("#new_user_auto_pop #user_email").val("");
            $("#new_user_auto_pop #user_email").blur();
            expect(objValidateAutoPop.items["user_email"].status).to.be.false;
        });

        it('Test 25 Case when all values are filled and something went wrong', function (done) {
            var post = sinon.stub($, 'ajax');
            post.yieldsTo('success', [true]);
            console.log('Test 25')
            $("#new_user_auto_pop #user_first_name").val("archit")
            $("#new_user_auto_pop #user_last_name").val("dugar")
            $("#new_user_auto_pop #user_email").val("archiasd")
            $("#new_user_auto_pop .submit").click();
            this.clock.tick(5);
            expect(objValidateAutoPop.status).to.be.false
            post.restore();
            done();
        });

        it('Test 26 Case when all values are filled and form submitted', function (done) {
            var post = sinon.stub($, 'ajax');
            post.yieldsTo('success', [true]);
            console.log('Test 26')
            $('#new_user_auto_pop').attr("action", "https://www.stitchfix.com/?utm_campaign=68ba126472fa2d9a8bb75a848dbd29cd&amp;&amp;utm_medium=other");
            $("#new_user_auto_pop #user_first_name").val("archit")
            $("#new_user_auto_pop #user_last_name").val("dugar")
            $("#new_user_auto_pop #user_email").val("archiasd@gmail.com")
            $("#new_user_auto_pop .rbtn-scale-auto-pop[value=119]").parent().addClass("active");
            $("#new_user_auto_pop .rbtn-scale-auto-pop[value=119]").prop("checked", true);
            $("#new_user_auto_pop .submit").click();
            this.clock.tick(10);
            expect(objValidateAutoPop.status).to.be.true
            post.restore();
            done();
        });

        it('Test 27 Case when all values are filled and form submitted with no query params in the action string', function (done) {
            var post = sinon.stub($, 'ajax');
            post.yieldsTo('success', [true]);
            console.log('Test 27')
            $('#new_user_auto_pop').attr("action", "https://www.stitchfix.com/");
            $("#new_user_auto_pop #user_first_name").val("archit")
            $("#new_user_auto_pop #user_last_name").val("dugar")
            $("#new_user_auto_pop #user_email").val("archiasd@gmail.com")
            $(".rbtn-scale-auto-pop").parent().removeClass("active");
            $("#new_user_auto_pop .rbtn-scale-auto-pop[value=39]").prop("checked", true);
            $("#new_user_auto_pop .rbtn-scale-auto-pop[value=39]").parent().addClass("active");
            $("#new_user_auto_pop .submit").click();
            this.clock.tick(10);
            expect(objValidateAutoPop.status).to.be.true
            post.restore();
