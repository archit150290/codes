...
describe('General Test', function () {
    ...
    describe("constructor", function () {
        ...
        it("Test 3 should have 2 variants", function () {
            expect(2).to.equal(Object.keys(testConfig).length);
        });

        it("Test 4 should have a variant from array['home-a','home-b']", function () {
            expect(['home-a','home-b']).to.include(ABalytics.variantName);
        });
        ...
        it("Test 6 should not have a variant from array['home-a','home-b']", function () {
        ...
        it("Test 10 TestConfig Variable should have variants home-a=95% , home-b= 5% Respectively", function () {
            //=====desktop other 
            expect(testConfig).to.have.property('home-a')
            expect(testConfig['home-a']).to.have.deep.property('min', 1)
            expect(testConfig['home-a']).to.have.deep.property('max', 95)
            expect(testConfig['home-a']).to.have.deep.property('variant_id', 0)
            expect(testConfig).to.have.property('home-b')
            expect(testConfig['home-b']).to.have.deep.property('min', 96)
            expect(testConfig['home-b']).to.have.deep.property('max', 100)
            expect(testConfig['home-b']).to.have.deep.property('variant_id', 1)
        });
    ...
    describe("Test 19 Fire ABalytics.generateToken get results", function () {
     ...
     describe("Check Ratios based upon utm_medium", function () {
        ...
        it("Test 21 TestConfig Variable should have variants as given Respectively where utm_medium == brand", function () {
            //=====desktop brand
            expect(testConfig).to.have.property('home-a')
            expect(testConfig['home-a']).to.have.deep.property('min', 1)
            expect(testConfig['home-a']).to.have.deep.property('max', 95)
        ...
        it("Test 22 TestConfig Variable should have variants  Respectively where Mobile utm_medium == brand", function () {
            //=======mobile brand
            if (window.__html__) {
                document.body.innerHTML = window.__html__['test/qa.welcome.html'];
            }
        ...
        it("Test 22-1 TestConfig Variable should have variants Respectively where Mobile utm_medium == brand", function () {
            //=======mobile other
            if (window.__html__) {
                document.body.innerHTML = window.__html__['test/qa.welcome.html'];
            }
        ...
        it('Test 23 Handle when utm_medium="" pass from querystring', function () {
            if (window.__html__) {
                document.body.innerHTML = window.__html__['test/qa.welcome.html'];
            }
    ...
    describe("All Anchor Tags Start from *.stitchfix.com have 3 utm params ->>", function () {
        ...
        it("Test 24 All Anchor tags containg utm_content", function () {
            $('a[href*=".stitchfix.com"]').each(function () {
                var testParam;
                var testAllowedParams = ["utm_source", "utm_medium", "utm_campaign"];
                var samepage = $(this).attr("href").indexOf('style.stitchfix.com/qa.html');
        ...
        it('Test 25 All form Action containg "utm_source", "utm_medium", "utm_campaign"', function () {
            $('form').each(function () {
                var testParam;
                var testAllowedParams = ["utm_source", "utm_medium", "utm_campaign"];
    ...
    describe("Check Switch condition based upon Varient ID", function () {
        ...
        it('Test 26 variantID=0 means HOME A', function () {
...
/*
        it('Test 27 variantID=1 HOME B', function () {
            ABalytics.variantId = 1;
            ABalytics.applyHtml();
            expect({}).to.be.empty;
        });
        it('Test 28 variantID=1 HOME B without site_q blank', function () {
            ABalytics.variantId = 1;
            site_q='';
            ABalytics.applyHtml();
            expect({}).to.be.empty;
