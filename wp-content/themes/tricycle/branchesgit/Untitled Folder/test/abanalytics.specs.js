...
describe('General Test', function () {
    ...
    describe("constructor", function () {
        ...
        it("Test 3 should have 21 variants", function () {
            expect(21).to.equal(Object.keys(testConfig).length);
        });

        it("Test 4 should have a variant from array['a','b','c','d','e','f', 'g', 'h','i']", function () {
            expect(['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 'f-banner', 'q-banner', 'h-banner']).to.include(ABalytics.variantName);
        });
        ...
        it("Test 10 TestConfig Variable should have variants e=10% , f= 30%, g=60% Respectively", function () {
            ...
            expect(testConfig.f).to.have.deep.property('max', 35)
            ...
            expect(testConfig.h).to.have.deep.property('min', 36)
            expect(testConfig.h).to.have.deep.property('max', 70)
            ...
            expect(testConfig.p).to.have.deep.property('min', 0)
            expect(testConfig.p).to.have.deep.property('max', 0)
            expect(testConfig.p).to.have.deep.property('variant_id', 15)
            expect(testConfig.p).to.have.deep.property('min', 0)
            expect(testConfig.p).to.have.deep.property('max', 0)
            expect(testConfig.p).to.have.deep.property('variant_id', 15)
            expect(testConfig.q).to.have.deep.property('min', 71)
            expect(testConfig.q).to.have.deep.property('max', 80)
            expect(testConfig.q).to.have.deep.property('variant_id', 16)
            expect(testConfig.r).to.have.deep.property('min', 0)
            expect(testConfig.r).to.have.deep.property('max', 0)
            expect(testConfig.r).to.have.deep.property('variant_id', 17)
            expect(testConfig['f-banner']).to.have.deep.property('min', 81)
            expect(testConfig['f-banner']).to.have.deep.property('max', 85)
            expect(testConfig['f-banner']).to.have.deep.property('variant_id', 18)
            expect(testConfig['h-banner']).to.have.deep.property('min', 86)
            expect(testConfig['h-banner']).to.have.deep.property('max', 95)
            expect(testConfig['h-banner']).to.have.deep.property('variant_id', 19)
            expect(testConfig['q-banner']).to.have.deep.property('min', 96)
            expect(testConfig['q-banner']).to.have.deep.property('max', 100)
            expect(testConfig['q-banner']).to.have.deep.property('variant_id', 20)
        });
    ...
    describe("Test Cases for Top promo banner Variant", function () {
        before(function () {
            if (window.__html__) {
                document.body.innerHTML = window.__html__['test/qa.html'];
            }
        ...
        it("Test 44 for variants f-banner", function () {
            $($("body")[0]).attr("class", "")
            ABalytics.variantId = 18;
            var clock = sinon.useFakeTimers();
            ABalytics.applyHtml();
            clock.tick(20);
            clock.restore();
            var ClassAppended = $($("body")[0]).hasClass("promos");
            expect({}).to.be.empty;
        })
        it("Test 45 for variants h-banner", function () {
            $($("body")[0]).attr("class", "")
            ABalytics.variantId = 19;
            var clock = sinon.useFakeTimers();
            ABalytics.applyHtml();
            clock.tick(30);
            clock.restore();
        ...
        it('Test 46 for variants h-banner', function () {
            ABalytics.variantId = 19;
            var clock = sinon.useFakeTimers();
            ABalytics.applyHtml();
            clock.tick(20);
        ...
        it('Test 47(h-banner) Select value for top form and get return value similar in new user form', function () {
            ABalytics.variantId = 19;
            var clock = sinon.useFakeTimers();
            ABalytics.applyHtml();
            $(".rbtn-scale-pop").parent().removeClass("active");
        ...
        it('Test 48(h-banner) Select When size chart is clicked', function () {
            $("table.size-chart td.s:last").click();
            ABalytics.variantId = 19;
            var clock = sinon.useFakeTimers();
            ABalytics.applyHtml();
        ...
        it('Test 49(h-banner) check if fb button is ours', function () {
            ABalytics.variantId = 19;
            var clock = sinon.useFakeTimers();
            ABalytics.applyHtml();
            clock.tick(550);
    ...
    describe("Test Cases for Experimenter calls", function () {
        ...
        it("Test 50 for Checking experimenter calls", function (done) {
            var post = sinon.stub($, 'ajax');
        ...
        it('Test 51 Call experimenter Failure', function (done) {
            var post = sinon.stub($, 'ajax');
            post.yieldsTo('error');
            ABalytics.getExperimenterVariant();
            expect(post.called).to.equal(true);
        ...
        it('Test 52 Call updateExperimenterResponse Failure', function (done) {
            var post = sinon.stub($, 'ajax');
            post.yieldsTo('error');
