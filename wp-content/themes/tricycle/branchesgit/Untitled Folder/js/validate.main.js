...
function check_uniqueness(item, frmSelector) {
    ...
    var tmpu = $(".link-login").first().attr("href").split("?");
    var offsetQ = "";
    if (tmpu.length > 1) {
        offsetQ = tmpu[1];
    }
...
$(document).on('click', '.size-chart td', function (event) {
    ...
    (typeof(sc)!="undefined" && sc == "topsizechart") ? $("#new_user .btn-group").parent(".form-group").css("display", "none") : "";
    $("#sizeChart-pop").fadeOut();
