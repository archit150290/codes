...
var ABalytics = (function (window, document, undefined) {
    ...
    return {
        ...
        init: function (config, slot) {
            ...
            // Always Update ratio sequence when ratio change
            var getRatio = function (r) {
                if (getCookie("is-top-promo") == "false") {
                    var r = getRandomInt(1, ABalytics.minB);
                } else if (getCookie("is-top-promo") == "true") {
                    var r = getRandomInt(ABalytics.minB, 101);
                }
                return r;
            }
            var bannerVariant = ["f-banner", "h-banner", "q-banner"];
            if (md.mobile() != null) {
                if (this.utm_medium == "brand") {
                    ...
                    testConfig = {"a": {"min": 0, "max": 0, "variant_id": 0},
                        ...
                        "f": {"min": 1, "max": 35, "variant_id": 5},
                        "g": {"min": 0, "max": 0, "variant_id": 6},
                        "h": {"min": 36, "max": 70, "variant_id": 7},
                        ...
                        "p": {"min": 0, "max": 0, "variant_id": 15},
                        "q": {"min": 71, "max": 80, "variant_id": 16},
                        "r": {"min": 0, "max": 0, "variant_id": 17},
                        "f-banner": {"min": 81, "max": 85, "variant_id": 18},
                        "h-banner": {"min": 86, "max": 95, "variant_id": 19},
                        "q-banner": {"min": 96, "max": 100, "variant_id": 20},
                        "s-banner": {"min": 0, "max": 0, "variant_id": 21},
                        "t-banner": {"min": 0, "max": 0, "variant_id": 22},
                    };
                } else {
                    //=====mobile other 
                    testConfig = {"a": {"min": 0, "max": 0, "variant_id": 0},
            ...
            if (checkBannerVariant(bannerVariant, testConfig)) {
                r = getRatio(r);
                ABalytics.VariantRequirement.setBanner(false);
            }
        ...
        applyHtml: function () {
            if (md.mobile() == null || md.tablet() != null || (this.variantId >= 0 && this.variantId <= 8) || (this.variantId >= 18 && this.variantId <= 19)) {
            ...
            switch (this.variantId) {
                ...
                case 15:
                    if (site_q == "") {
                        site_q = "?utm_campaign=" + dfutm_camid + "&utm_source=ampush&utm_medium=other";
                    }
                    window.location = 'https://style.stitchfix.com/home1' + site_q;
                    break;
                ...
                case 16:
                    ...
                    window.location = 'https://style.stitchfix.com/home4' + site_q;
                    break;
                case 17:
                    if (site_q == "") {
                        site_q = "?utm_campaign=" + dfutm_camid + "&utm_source=ampush&utm_medium=other";
                    }
                    window.location = 'https://style.stitchfix.com/home5' + site_q;
                    break;
                case  18:
                    ABalytics.VariantRequirement.VariantF();
                ...
                case  19:
                    ...
                    break;
                case  20:
                    ABalytics.VariantRequirement.setBanner(true);
                    if (site_q == "") {
                        site_q = "?utm_campaign=" + dfutm_camid + "&utm_source=ampush&utm_medium=other";
                    }
                    window.location = 'https://style.stitchfix.com/home4-banner' + site_q;
                    break;
                case  21:
                    ABalytics.VariantRequirement.setBanner(true);
                    if (site_q == "") {
                    ...
                    window.location = 'https://style.stitchfix.com/about6-banner' + site_q;
                    break;
                case  22:
                    ABalytics.VariantRequirement.setBanner(true);
                    if (site_q == "") {
                    ...
                    window.location = 'https://style.stitchfix.com/about7-banner' + site_q;
                    break;
            }
        }
    };
...
ABalytics.VariantRequirement = {
    ...
    setBanner: function (status) {
        document.cookie = "is-top-promo=" + status + ";";
    },
    topPromoBanner: function () {
        ABalytics.paintOnLoad("#top-promo", function () {
            if (getCookie("top-promo-hidden") == "") {
                ABalytics.VariantRequirement.setBanner(true);
                $("body").addClass("promos");
            }
        }, 20);
    }
}
...
function checkBannerVariant(banners, items) {
    var isBanner = false;
    for (var x in items) {
        if ($.inArray(x, banners) != -1) {
            if (items[x]["max"] > 0 && items[x]["min"] > 0) {
                ABalytics.minB = items[x]["min"];
                isBanner = true;
                break;
            }
        }
