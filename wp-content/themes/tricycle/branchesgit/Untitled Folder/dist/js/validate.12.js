...
function check_uniqueness_autopop(item) {
    check_uniqueness(item, "#new_user_auto_pop");
}
...
function getErrTag(msg) {
    return "<div class='error-msg' style='color:#F05D61;font-size:11pt'><em><span>" + msg + "</span></em></div>";
}
...
$(document).on('click', '.size-chart td', function (event) {
    var validClass = {"xs": 37, "s": 38, "m": 39, "l": 40, "xl": 41, "xxl": 119};
    for (var cc in validClass) {
        if ($(this).hasClass(cc)) {
            $(".rbtn-scale").prop("checked", false);
            $(".rbtn-scale-pop").prop("checked", false);
            $(".rbtn-scale-pop").parent().removeClass("active");
            $(".rbtn-scale-pop[value=" + validClass[cc] + "]").prop("checked", true);
            $(".rbtn-scale-pop[value=" + validClass[cc] + "]").parent().addClass("active");
            $(".rbtn-scale").parent().removeClass("active");
            $(".rbtn-scale[value=" + validClass[cc] + "]").prop("checked", true);
            $(".rbtn-scale[value=" + validClass[cc] + "]").parent().addClass("active");
            break;
        }
    }
    objValidate.applyValidation("client_scale");
    var sc = $(this).parents("tbody").attr("data-whichsc")
    if(ABalytics.variantId==7 && sc == "topsizechart"){
...
$(document).on('click', "#new_user_pop .submit", function (event) {
    ...
    if ($(".rbtn-scale-pop[type=radio]:checked").length < 1) {
        $(".rbtn-scale-pop").parent().removeClass("active");
        $(".rbtn-scale-pop[value=39]").prop("checked", true);
...
//======= Auto PopUp
var itemsConfigAutoPop = {
    "user_first_name": {
        "selector": "#new_user_auto_pop #user_first_name"
        , "rules": {"empty": {"message": "This is a required field"}}
        , "validate_on_blur": true
    },
    "user_last_name": {
        "selector": "#new_user_auto_pop #user_last_name"
        , "rules": {"empty": {"message": "This is a required field"}}
    },
    "user_email": {
        "selector": "#new_user_auto_pop #user_email"
        , "rules": {
            "empty": {"message": "This is a required field"},
            "email": {"message": "Email is not a valid email address."},
            "branded_email": {"message": "You must enter a valid email address. Please check your entry."},
            "success_callback": {"name": "check_uniqueness_autopop"},
        },
    },
    "client_scale": {
        "selector": "input[type='radio'].rbtn-scale-auto-pop"
        , "rules": {
            "radio_empty": {"message": "This is a required field"},
        },
    },
    "user_client_attributes_shipping_postcode": {
        "selector": "#new_user_auto_pop #user_client_attributes_shipping_postcode"
        , "rules": {
            "empty": {"message": "This is a required field"},
            "us_zip": {"message": "Zip Code is not a valid U.S. postal code. Currently, we only ship to U.S. addresses."},
        }
...
var objValidateAutoPop = new validate(itemsConfigAutoPop);
$(document).on('blur', itemsConfigAutoPop.user_first_name.selector, function (event) {
    objValidateAutoPop.applyValidation("user_first_name");
});

$(document).on('blur', itemsConfigAutoPop.user_last_name.selector, function (event) {
    objValidateAutoPop.applyValidation("user_last_name");
});

$(document).on('blur', itemsConfigAutoPop.user_email.selector, function (event) {
    ifblur = 1;
    emailPerfect = true;
    objValidateAutoPop.applyValidation("user_email");
});

$(document).on('click', "#new_user_auto_pop .btn-group", function (event) {
    objValidateAutoPop.applyValidation("client_scale");
});

$(document).on('click', "#new_user_auto_pop .submit", function (event) {
    uniqueness_checked = 0;
    emailPerfect = true;
    event.preventDefault();
    objValidateAutoPop.fire();
    var checkForm = setInterval(function () {
        if (uniqueness_checked == 1 && objValidateAutoPop.status == true && emailPerfect == true) {
            clearInterval(checkForm);
            var url = $('#new_user_auto_pop').attr("action");
            var urlParams = getQueryParams(url);
            if (!("utm_source" in urlParams) || urlParams["utm_source"] != "ampush") {
                if ($.trim(url).indexOf("?") >= 0) {
                    urlParams["utm_source"] = "ampush";
            ...
            $(this).find("input[type=submit]").val("SIGNING UP...");
            eventONSubmit($("#new_user_auto_pop").serialize());
            chReferer();
            $("#new_user_auto_pop").submit();
            return objValidateAutoPop.status;

        } else if (emailPerfect == false || objValidateAutoPop.status == false) {
            clearInterval(checkForm);
        }
    }, 300)
});
