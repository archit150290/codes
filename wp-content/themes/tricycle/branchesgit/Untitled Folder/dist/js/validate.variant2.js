...
function check_uniqueness(item, frmSelector) {
    ...
    var tmpu = $(".neutral-link").first().attr("href").split("?");
    var offsetQ = "";
    if (tmpu.length > 1) {
        offsetQ = tmpu[1];
    }
...
validate.prototype.checkRadioEmpty = function (itemName) {
    var item = this.items[itemName];
    var currentOption = $("input[name='user[business_line]']:checked").val();
    var status = false;
    var selector = item.selector;
    if(currentOption == "Womens"){
        selector = "#women-size-buttons "+selector;
    }else{
        selector = "#men-size-buttons "+selector;
    }
...
$(document).on('click', "#new_user #women-size-buttons", function (event) {
    objValidate.applyValidation("client_scale");
});

$(document).on('click', "#new_user #men-size-buttons", function (event) {
    objValidate.applyValidation("client_scale");
});
...
$(document).on('click', "#new_user .submit", function (event) {
    ...
    var check = setInterval(function () {
        if (uniqueness_checked == 1 && objValidate.status == true && emailPerfect == true) {
            $("#new_user .submit").val("Signing up...")
            clearInterval(check);
...
$(document).on('click', '.size-chart td', function (event) {
    ...
    for (var cc in validClass) {
        if ($(this).hasClass(cc)) {
            $("#women-size-buttons .rbtn-scale").prop("checked", false);
            $("#women-size-buttons .rbtn-scale[value=" + validClass[cc] + "]").prop("checked", true);
            break;
        }
    }
