...
<div class='wrap'>
    ...
    <div class='archive_dharmatalks'>
        ...
        <header class='rubric_mid dharmatalks'>
            <h1>Archive</h1>
            <?php
            $last = wp_get_recent_posts(array('post_type'=>DHARMATALKS_POST_TYPE,'posts_per_page'=>1,'post_status'=>'publish'));
            $last_post = $last['0']['post_date'];
            $last_post_year = date('Y', strtotime($last_post));
            $first = wp_get_recent_posts(array('post_type'=>DHARMATALKS_POST_TYPE,'posts_per_page'=>1,'post_status'=>'publish', 'orderby'=>'post_date', 'order'=>'ASC'));
            $first_post = $first['0']['post_date'];
            $first_post_year = date('Y', strtotime($first_post));
            ...
            <div class='dropdown'>
                <div class='dropdown-container'>
                    ...
                    <ul class='dropdown-menu dropdown-select'>
                        <?php
                        for ($i = $currentYear; $i >= $first_post_year; $i--) {
                            ?>
                    ...
                    <article class='dharmatalks'>
                        <figure>
                            <a href='<?php echo $permalink; ?>'>
                                <div class='icon-video_play' data-grunticon-embed></div>
                                <?php echo get_image_with_srcset(array("image_type"=>"thumbnail", "post_id"=>$postid, "source_width"=>array(2000, 1000, 500), "size"=>"image-size-500", "attr_sizes"=>'(min-width: 50em) 33vw,100vw')); ?>
