...
jQuery(document).ready(function () {
    ...
    var checkdiv =setInterval(function(){
        if(typeof(document.getElementsByClassName("gsc-search-button gsc-search-button-v2")[0])!="undefined" && document.getElementsByClassName("gsc-search-button gsc-search-button-v2")[0].src=="http://www.google.com/uds/css/v2/search_box_icon.png"){
            document.getElementById("loadersearch").remove();
            document.getElementsByClassName("gsc-search-button gsc-search-button-v2")[0].src=searchimagesrc;
            document.getElementsByClassName("searchbox")[0].style.display="block";
            clearInterval(checkdiv);
        }
        
    },10);
    //change on mouseover and mouseour google image
    var changesrc =setInterval(function(){
        if(typeof(document.getElementsByClassName("gsc-search-button gsc-search-button-v2")[0])!="undefined" && document.getElementsByClassName("gsc-search-button gsc-search-button-v2")[0].src==searchimagesrc){
             $('.gsc-search-button .gsc-search-button-v2').on('mouseover',function(){
                document.getElementsByClassName("gsc-search-button gsc-search-button-v2")[0].src=searchimagesrcnew;
             });
             $('.gsc-search-button .gsc-search-button-v2').on('mouseout',function(){
                document.getElementsByClassName("gsc-search-button gsc-search-button-v2")[0].src=searchimagesrc;
             });
            clearInterval(changesrc);
        }
    },10);
    
    
    /*
     * to check if view trailer link is present in film club or not
     */
    if(jQuery('article.article_base.filmclub').length){
        if((jQuery('.view-trailer').length == 0) && (jQuery('#latestpostfc').val()!=1)){
            jQuery('.icon-video_play').css('visibility','hidden');
            jQuery('.article-video_featured figure').hover(function(){
                jQuery('.article-video_featured figure').css( 'cursor', 'default' );
            }); 
        }
    }
    ...
    $('#interstitial-ad-enews').submit(function(e) {
        e.preventDefault();
        var enewsemail = $("#interstitial-ad-email").val();
        modal('action');
        $("#mce-EMAIL").val(enewsemail);
        $(".interstitial-ad-modal .modal-fade-screen").removeClass("modal-visible");
        //modal('interstitial-ad');
    });
