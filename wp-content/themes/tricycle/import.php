...
<?php
global $wpdb;
//'post', 'magazine', 'filmclub', 'filmfestival', 'ebooks', 'dharmatalks'
/*$paged = isset($_GET['pnum']) ? $_GET['pnum'] : 1;
$args = array('post_type' => array('dharmatalks'), 'orderby'=>'ID', 'order'=>'DESC', 'posts_per_page'=>200, 'paged'=>$paged);
$result = new WP_Query( $args );
if($result->have_posts())
{
    $no_comment_count = 0;
    $no_wordpress_id_count = 0;
    $no_drupal_id_count = 0;
    $comment_added_count = 0;
    while($result->have_posts())
    {
        $result->the_post();
        $postId = get_the_ID();
        $mapped_row = $wpdb->get_row("SELECT * FROM try_wp_drupal_mapping_table WHERE status = 0 AND worpress_id = ".$postId);
        
        if($mapped_row)
        {
            $drupal_id = $mapped_row->drupal_id;
            $mapped_row_id = $mapped_row->id;
            
            if($drupal_id)
            {
                $comments = $wpdb->get_results("SELECT * FROM comments WHERE nid = $drupal_id");
                ...
                if($comments)
                {
                    ...
                    $comment_added_count++;
                    $wpdb->query("UPDATE try_wp_drupal_mapping_table SET status = 1 WHERE id = ".$mapped_row_id);
                }
                else
                {
                    echo "No comments. Wordpress ID: ".$postId."<br/>";
                    $no_comment_count++;
                }
            }
            else
            {
                echo "Drupal ID is missing. Wordpress ID: ".$postId."<br/>";
                $no_drupal_id_count++;
            }
        }
        else
        {
            echo "Wordpress ID is missing. Wordpress ID: ".$postId."<br/>";
            $no_wordpress_id_count++;
        }
    }
    echo "Total Count (No comments): ".$no_comment_count."</br>";
    echo "Total Count (No wordpress id): ".$no_wordpress_id_count."</br>";
    echo "Total Count (No drupal id): ".$no_drupal_id_count."</br>";
    echo "Total Comment Count (Added): ".$comment_added_count."</br>";
}*/
