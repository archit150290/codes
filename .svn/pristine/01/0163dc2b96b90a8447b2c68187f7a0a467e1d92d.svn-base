	...
	public static function handleError($code, $description, $file = null, $line = null, $context = null) {
		...
		if ($debug) {
			...
			return Debugger::getInstance()->outputError($data);
				...
				$email = array("to" => "rmehta@arcgate.com", "subject" => $subject, "config_vars" => $configVars, "template" => "error_notification");
				$errorEmailModel->save($email);
				//$Email = new CakeEmail();
				//$Email->subject(__d('utils', 'Error notification from %s', env('HTTP_HOST')))
				//	->from(array('error@' . env('HTTP_HOST') => 'Error Handler'))
				//	->to("rmehta@arcgate.com")
				//	->emailFormat('html')
				//	->template('error_notification');
				//	
				//$Email->viewVars(compact('error','code', 'description', 'file', 'line', 'context', 'session', 'server', 'request', 'trace'));
				//$Email->send();
		}
