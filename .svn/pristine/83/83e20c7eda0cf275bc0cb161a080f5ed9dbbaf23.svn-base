...
class ImageComponent extends Component {
	...
	var $thumb_subloc = '/files/uploaded/img/thumbs/';
	...
	function uploadFile ($file, $resize_data = array()) {
		...
		$dim_setting = $dim1 = $dim2 = null; $rename_flag = false;
		...
		// resize the image
                //function resize($src, $dim1, $dim2=true, $thumb_subloc, $rename_flag = false, $dim_setting = RESIZE_BOTH) 
		$errors = $this->resize($this->resizeTmpFile, $dim1, $dim2, $this->thumb_subloc, $rename_flag,$dim_setting);
	...
	*/
	function addPostfixTofilename($file_name, $postfix)
	{
		...
		return $firstpart.'_'.$postfix.'.'.$ext;
	}
	...
	function resize($src, $dim1, $dim2=true, $thumb_subloc, $rename_flag = false, $dim_setting = RESIZE_BOTH) 
	{
		...
		if(1)
		{
			...
			if ($dim_setting == "width") 
			{
				...
				$thumb_imgname = $this->addPostfixTofilename($src_img_name, 'width_'.$newWidth.'x'.$newHeight);
			}
			else if ($dim_setting == "height") 
			{
				...
				$thumb_imgname = $this->addPostfixTofilename($src_img_name, 'height_'.$newWidth.'x'.$newHeight);
			} 
