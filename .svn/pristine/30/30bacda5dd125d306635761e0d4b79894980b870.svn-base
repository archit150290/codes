<?php
define("MAGAZINE_POST_TYPE", "magazine");
define("MAGAZINE_POST_ISSUE_TAXONOMY", "magazine-issue");
define("MAGAZINE_POST_DEPARTMENT_TAXONOMY", "magazine-department");
...
define("BASE_HERO_TEMPLATE_FEATURED_IMAGE", "base_hero_template_featured_image");
define("FEATURED_IMAGE_AS_BASE_HERO_TEMPLATE_FEATURED_IMAGE", "featured_image_as_base_hero_template_featured_image");
...
function custom_post_type_and_taxonomy_init() {
    ...
    $magazine_issue_taxonomy_args = array(
        ...
        'query_var' => true,
        'rewrite' => array('slug' => MAGAZINE_POST_ISSUE_TAXONOMY, 'with_front' => false)
    );

    register_taxonomy(MAGAZINE_POST_ISSUE_TAXONOMY, array(MAGAZINE_POST_TYPE), $magazine_issue_taxonomy_args);
    //Magazine Issue taxonomy ends here
    // Add new taxonomy, make it hierarchical (like categories)
    
    //Article post type starts here
    $labels = array(
        'name' => _x('Magazine', 'post type general name', 'your-plugin-textdomain'),
    ...
    $args = array(
        ...
        'show_ui' => true,
        'show_in_menu' => true,
        // 'rewrite' => array('slug' => MAGAZINE_POST_TYPE . "/%" . MAGAZINE_POST_DEPARTMENT_TAXONOMY . "%", 'with_front' => false),
        'rewrite' => array('slug' => MAGAZINE_POST_TYPE, 'with_front' => false),
    ...
    $magazine_department_labels = array(
        'name' => _x('Magazine Departments', 'taxonomy general name'),
        'singular_name' => _x('Magazine Department', 'taxonomy singular name'),
        'search_items' => __('Search Magazine Department'),
        'all_items' => __('All Magazine Departments'),
        'parent_item' => __('Parent Magazine Department'),
        'parent_item_colon' => __('Parent Magazine Department:'),
        'edit_item' => __('Edit Magazine Department'),
        'update_item' => __('Update Magazine Department'),
        'add_new_item' => __('Add New Magazine Department'),
        'new_item_name' => __('New Magazine Department Name'),
    ...
    $magazine_department_args = array(
        'hierarchical' => true,
        'labels' => $magazine_department_labels,
        'show_ui' => true,
        'show_admin_column' => true,
        'query_var' => true,
        'rewrite' => array('slug' => MAGAZINE_POST_DEPARTMENT_TAXONOMY, 'with_front' => false)
    );

    register_taxonomy(MAGAZINE_POST_DEPARTMENT_TAXONOMY, array(MAGAZINE_POST_TYPE), $magazine_department_args);
    //Article category taxonomy ends here

    $parent_term = term_exists(DEFAULT_ARTICLE_CATEGORY_TAXONOMY_TERM_NAME, MAGAZINE_POST_DEPARTMENT_TAXONOMY); // array is returned if taxonomy is given
    if (!$parent_term) {
        $parent_term_id = $parent_term['term_id']; // get numeric term id
        wp_insert_term(
                DEFAULT_ARTICLE_CATEGORY_TAXONOMY_TERM_NAME, // the term 
                MAGAZINE_POST_DEPARTMENT_TAXONOMY, // the taxonomy
    ...
    register_taxonomy(TOPIC_TAXONOMY, array("post", MAGAZINE_POST_TYPE, FILM_FESTIVAL_POST_TYPE, DHARMATALKS_POST_TYPE, EBOOKS_POST_TYPE, FILM_CLUB_POST_TYPE), $topic_taxonomy_args);

    //Topic taxonomy starts here
   
    //Film Club post type starts here
    $film_club_labels = array(
        'name' => _x('Film Club', 'post type general name', 'your-plugin-textdomain'),
        'singular_name' => _x('Film Club', 'post type singular name', 'your-plugin-textdomain'),
        'menu_name' => _x('Film Club', 'admin menu', 'your-plugin-textdomain'),
    ...
    $film_club_args = array(
        ...
        'show_ui' => true,
        'show_in_menu' => true,
        'rewrite' => array('slug' => FILM_CLUB_POST_TYPE, 'with_front' => false),
        'capability_type' => 'post',
        'has_archive' => true,
        'hierarchical' => false,
        'menu_position' => null,
    ...
    register_post_type(FILM_CLUB_POST_TYPE, $film_club_args);
...
function article_category_permalink_structure($post_link, $post, $leavename, $sample) {
    if (false !== strpos($post_link, '%' . MAGAZINE_POST_DEPARTMENT_TAXONOMY . '%')) {
        $article_category_type_term = get_the_terms($post->ID, MAGAZINE_POST_DEPARTMENT_TAXONOMY);
        if (!empty($article_category_type_term))
            $post_link = str_replace('%' . MAGAZINE_POST_DEPARTMENT_TAXONOMY . '%', array_pop($article_category_type_term)->slug, $post_link);
        else
            $post_link = str_replace('%' . MAGAZINE_POST_DEPARTMENT_TAXONOMY . '%', '', $post_link);
    }
    return $post_link;
}
...
function magazine_issue_admin_posts_filter_restrict_manage_posts() {
    ...
    if (MAGAZINE_POST_TYPE == $type) {
        $tax_obj = get_taxonomy(MAGAZINE_POST_ISSUE_TAXONOMY);
...
function magazine_issue_posts_filter($query) {
    ...
    if ($type == MAGAZINE_POST_TYPE && is_admin() && $pagenow == 'edit.php' && isset($_GET[MAGAZINE_POST_ISSUE_TAXONOMY]) && $_GET[MAGAZINE_POST_ISSUE_TAXONOMY] != '') {
        if ($_GET[MAGAZINE_POST_ISSUE_TAXONOMY] == 'none') {
            $var = &$query->query_vars[MAGAZINE_POST_ISSUE_TAXONOMY];
            $taxonomy_terms = get_terms(MAGAZINE_POST_ISSUE_TAXONOMY, array(
                'hide_empty' => 0,
                'fields' => 'ids'
            ));
            ...
            $var = &$query->query_vars[MAGAZINE_POST_ISSUE_TAXONOMY];
            if (isset($var) && $var != 0) {
                $term = get_term_by('id', $var, MAGAZINE_POST_ISSUE_TAXONOMY);
                $var = $term->slug;
            }
        }
    }

    if (($type == MAGAZINE_POST_TYPE || $type == "post") && is_admin() && $pagenow == 'edit.php' && isset($_GET[TOPIC_TAXONOMY]) && $_GET[TOPIC_TAXONOMY] != '') {
...
function add_article_meta_boxes() {
    add_meta_box('home_featured_articles_metabox_id', __('Set as Home Page Marquee'), 'add_home_featured_articles_meta_box', MAGAZINE_POST_TYPE, 'normal', 'default');
    add_meta_box('article_layout_metabox_id', 'Layout', 'add_layout_meta_box', MAGAZINE_POST_TYPE, 'normal', 'default');
    add_meta_box('current_issues_articles_metabox_id', __('Set as Current Issues'), 'add_current_issues_articles_meta_box', MAGAZINE_POST_TYPE, 'normal', 'default');
    add_meta_box('article_Byline_authors', 'Byline Lead-in Text', 'add_article_authors_metabox', MAGAZINE_POST_TYPE, 'normal', 'high');
    add_meta_box('article_template_featured_image', 'Article Featured Image', 'add_article_template_featured_metabox', MAGAZINE_POST_TYPE, 'side', 'default');
}

/**
 * This function is used to add a metabox of Base or Hero Template Featured Image
 * @param type $post
 */
function add_article_template_featured_metabox($post)
{
    $base_hero_template_featured_image_url = "";
    $base_hero_template_featured_image_id = "";
    $select_checkbox = "";
    
    $base_hero_template_featured_image = get_base_hero_template_featured_image($post->ID);

    if($base_hero_template_featured_image)
    {
        $base_hero_template_featured_image_url = isset($base_hero_template_featured_image['featured_image_url']) ? $base_hero_template_featured_image['featured_image_url'] : "";
        $base_hero_template_featured_image_id = isset($base_hero_template_featured_image['ID']) ? $base_hero_template_featured_image['ID'] : "";
        
        if(isset($base_hero_template_featured_image['use_default_featured_image']) && $base_hero_template_featured_image['use_default_featured_image'] == 1)
        {
    ...
    ?>
    <?php
    if($base_hero_template_featured_image_url)
    {
        ?>
        <img class="base_hero_template_featured_image" style="width: 100%; float:left; margin: 0px 10px 0px 0px;" src="<?php echo $base_hero_template_featured_image_url; ?>"/>
        <input class="base_hero_template_featured_image_url" type="hidden" name="base_hero_template_featured_image_url" value="<?php echo $base_hero_template_featured_image_url; ?>" />
        <input class="base_hero_template_featured_image_id" type="hidden" name="base_hero_template_featured_image_id" value="<?php echo $base_hero_template_featured_image_id; ?>" />
        <a href="javascript:void(0);" class="remove_ad_photo" onclick="remove_template_featured_image();">Remove featured image</a>
        <?php
    }
    else
    {
        ?>
        <img class="base_hero_template_featured_image" style="width: 100%; float:left; margin: 0px 10px 0px 0px; display:none;" />
        <input class="base_hero_template_featured_image_url" type="hidden" name="base_hero_template_featured_image_url" />
        <input class="base_hero_template_featured_image_id" type="hidden" name="base_hero_template_featured_image_id" />
        <a href="javascript:void(0);" class="remove_ad_photo" onclick="remove_template_featured_image();" style="display: none;">Remove featured image</a>
        <?php
    }            
    ?>
    <a href="javascript:void(0);" id="template_featured_image">Set featured image</a>
    <p><label><input <?php echo $select_checkbox; ?> type="checkbox" name="featured_image_as_base_hero_template_featured_image" value="1" />Use Default Featured Image</label></p>
    <script type="text/javascript">
        jQuery(document).ready(function() {
            jQuery("#postimagediv h3 span").text("Default Featured Image");
            jQuery(document).on('click', '#template_featured_image', function(e) {
                var image_src_el = "base_hero_template_featured_image";
                var photo_el = "base_hero_template_featured_image_url";
                var photo_id = "base_hero_template_featured_image_id";
                e.preventDefault();
                var custom_uploader = wp.media({
                    title: 'Template featured image',
                    button: {
                        text: 'Set template featured image'
                    },
                    multiple: false
                })
                .on('select', function() {
                    ...
                    jQuery('.'+photo_el).val(attachment.url);
                    jQuery('.'+photo_id).val(attachment.id);
                    jQuery(".remove_ad_photo").show();
                })
                .open();
            });
        });

        function remove_template_featured_image()
        {
            jQuery(".base_hero_template_featured_image").hide().attr("src", "");
            jQuery(".base_hero_template_featured_image_url, .base_hero_template_featured_image_id").val("");
            jQuery(".remove_ad_photo").hide();
        }
    </script>
    <?php
}
...
/**
 * This function returns the featured image of base or hero template.
 * @param integer $post_id
 * @return string
 */
function get_base_hero_template_featured_image($post_id)
{
    $base_hero_template_featured_image = get_post_meta($post_id, BASE_HERO_TEMPLATE_FEATURED_IMAGE, TRUE);
    $is_featured_image_set_as_base_and_hero_template_featured_image = get_post_meta($post_id, FEATURED_IMAGE_AS_BASE_HERO_TEMPLATE_FEATURED_IMAGE, TRUE);
    ...
    if($base_hero_template_featured_image)
    {
        ...
        //$attachment_meta_data = wp_get_attachment_metadata($base_hero_template_featured_image['attachment_id']);
        ...
        $attachment_meta_data['ID'] = $attachment_id;
        $attachment_meta_data['featured_image_url'] = $upload_baseurl.'/'.$base_hero_template_featured_image['attachment_image'];
        $attachment_meta_data['alt'] = $image_alt;
        $attachment_meta_data['caption'] = $image_caption;
        $attachment_meta_data['description'] = $image_description;
        $attachment_meta_data['title'] = $image_title;
        $attachment_meta_data['use_default_featured_image'] = 1;
...
function add_article_authors_metabox($post) {
    ...
    <input type="text" placeholder="Add custom byline text" style="width: 85%" name="article_authors_name_metabox" value="<?php echo $article_authorname; ?>">
    <br/>
 ...
 */
function parent_childrelationship()
{
    $terms = get_terms("magazine-department");
    $uni_parent = array();
    foreach($terms as $department)
    {   
      $uni_parent[$department->term_id] = $department->parent;
    }
    ?>
    <input type="hidden" id="uniparent" value='<?php echo json_encode($uni_parent);?>'>
    <?php 
}
add_action('admin_head', 'parent_childrelationship');
...
function magazine_issue_taxonomy_custom_fields($tag) {
    ...
    <tr class="form-field">  
        ...
        <td>  
            <input type="text" name="term_meta[volume]" id="term_meta[volume]" size="25" style="width:60%;" value="<?php if(!empty($term_meta['volume'])) { echo $term_meta['volume']; } ?>"><br />  
        </td>  
    </tr>
     ...
     <tr class="form-field">  
        ...
        <td>  
            <input type="text" name="term_meta[number]" id="term_meta[number]" size="25" style="width:60%;" value="<?php if(!empty($term_meta['number'])) { echo $term_meta['number']; } ?>"><br />  
        </td>  
    </tr>
    <tr class="form-field">  
        <th scope="row" valign="top">  
            <label for="presenter_id"><?php _e('Buy Issue URL'); ?></label>  
        </th>  
        <td>  
            <input type="text" name="term_meta[buyissuelink]" id="term_meta[buyissuelink]" size="25" style="width:60%;" value="<?php if(!empty($term_meta['buyissuelink'])) { echo $term_meta['buyissuelink']; } ?>"><br />  
        </td>  
    </tr>
    <?php
}
...
add_action(MAGAZINE_POST_ISSUE_TAXONOMY.'_edit_form_fields', 'magazine_issue_taxonomy_custom_fields', 10, 2);
 ...
 */
add_action('edited_'.MAGAZINE_POST_ISSUE_TAXONOMY, 'save_taxonomy_custom_fields', 10, 2);
...
function article_save_meta_box_data($post_id) {
    ...
    // Get the terms
    $terms = wp_get_post_terms($post_id, MAGAZINE_POST_DEPARTMENT_TAXONOMY);

    // Only set default if no terms are set yet
    if (!$terms) {
        // Assign the default category
        $default_term = get_term_by('slug', 'columns', MAGAZINE_POST_DEPARTMENT_TAXONOMY);
        wp_set_post_terms($post_id, array($default_term->term_id), MAGAZINE_POST_DEPARTMENT_TAXONOMY);
    }
...
//    update_post_meta($post_id, MAGAZINE_RELATED_ARTICLES_META_KEY, $related_artiles_list);
    ...
    /* Condition to insert menu order if current issue is selected */
    if($current_issue_post == "1")
    {
        if(!metadata_exists( 'post', $post_id, "menu_order" ))
        {
            update_post_meta($post_id, "menu_order", "");
        }
 ...
 */
add_action('save_post_' . MAGAZINE_POST_TYPE, 'article_save_meta_box_data');
...
function topic_admin_posts_filter_restrict_manage_posts() {
    ...
    if ('post' == $type || MAGAZINE_POST_TYPE == $type) {
...
/**
 * This action will call the javascript in admin section.
 */
add_action( 'admin_enqueue_scripts', 'tricycle_custom_admin_javascript' );

/**
 * This function will include admin-javascript.js in the admin section.
 * @param string $hook
 */
function tricycle_custom_admin_javascript($hook) 
{
    wp_enqueue_script('tricycle_admin_script', get_template_directory_uri() . '/js/admin-javascript.js');
}?>