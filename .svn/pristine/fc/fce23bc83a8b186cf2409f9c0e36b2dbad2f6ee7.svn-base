...
<div class='wrap'>
    ...
    <div class='archive_magazine'>
	    ...
	    $topicsYear = get_magazine_issue_terms();
            $countTotal = count($topicsYear)-1;
	    if($topicsYear && preg_replace("/[^0-9]/", '', $topicsYear[0]->name))
	    {
		$latest_issue_year =  preg_replace("/[^0-9]/", '', $topicsYear[0]->name);
	    }else{
		$latest_issue_year =  date('Y');
	    }
            if($topicsYear && preg_replace("/[^0-9]/", '', $topicsYear[$countTotal]->name))
	    {
		$oldest_issue_year =  preg_replace("/[^0-9]/", '', $topicsYear[$countTotal]->name);
	    }else{
                $oldest_issue_year =  1991;
	    }
            ...
            <div class='dropdown'>
                <div class='dropdown-container'>
                    ...
                    <ul class='dropdown-menu dropdown-select'>
                        ...
                        <?php
                        for ($i = $currentYear; $i >= $oldest_issue_year; $i--) {
                            ?>
