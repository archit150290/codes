...
// Add default posts and comments RSS feed links to head.
    ...
    add_image_size('image-size-2001', 2001);
    add_image_size('image-size-2000', 2000, 1380, true);
    add_image_size('image-size-1001', 1001);
    add_image_size('image-size-1000', 1000, 690, true);
    add_image_size('image-size-800', 800);
    add_image_size('image-size-560', 560);
    add_image_size('image-size-501', 501);
    add_image_size('image-size-500', 500, 345, true);
    add_image_size('image-size-400', 400);
    add_image_size('image-size-280', 280);
...
include_once 'functions-tricycle-settings.php';
...
function add_post_meta_boxes() {
    add_meta_box('trikedaily_Byline_authors', 'Byline Lead-in Text', 'add_trikedaily_authors_metabox', 'post', 'normal', 'high');
    ...
    add_meta_box('article_template_featured_image', 'Article Featured Image', 'add_article_template_featured_metabox', "post", 'side', 'default');
    add_meta_box('trikedaily_audio_1', 'Audio', 'add_trikedaily_audio', 'post', 'normal', 'default');
}
...
function add_trikedaily_authors_metabox($post) {
    ...
    ?>
    <label>Add custom byline text followed by "[author#]". Multiple bylines are separated by commas. <br/></label>
    <input type="text" placeholder="Add custom byline text" style="width: 85%" name="trikedaily_authors_name_metabox" value="<?php echo $trikedaily_authorname; ?>">
    <br>
    <label>(For example: By [author1], Edited by [author2]) </label>
    <?php
}
...
function add_trikedaily_audio($post) {
    $upload_dir = wp_upload_dir();
    ?>
    <script type="text/javascript">
        function upload_meta_image(title, textboxinput)
        {
            ...
            meta_image_frame = wp.media.frames.meta_image_frame = wp.media({
                ...
                multiple: false
                    ...
                    .on('select', function () {
                        ...
                        jQuery("#" + textboxinput).val(attachment.url);
                    })
    ...
    <?php
    $tridedaily_author1 = get_post_meta($post->ID, "trikdedaily_meta_audio1", true);
    if (!empty($tridedaily_author1)) {
        $upload_baseurl = $upload_dir['baseurl'] . '/' . $tridedaily_author1;
    } else {
        $upload_baseurl = "";
    }
    $htmlaudio1 = '<p>';
    $htmlaudio1 .= 'Audio 1';
    $htmlaudio1 .= '</p>';
    $htmlaudio1 .= '<input id="upload_file" type="hidden" size="36" name="upload_file" value="" />';
    $htmlaudio1 .= '<input readonly id="upload_file_audio1" type="text" size="90%" name="upload_file_audio1" value = "' . $upload_baseurl . '"  />';
    $htmlaudio1 .='<input type="button" onclick = upload_meta_image("upload-Audio1-file","upload_file_audio1") id="upload_file_button_audio1" value="Upload Audio 1" />';
    echo $htmlaudio1 . "<br>";
    ?>


    <?php
    $tridedaily_author2 = get_post_meta($post->ID, "trikdedaily_meta_audio2", true);
    if (!empty($tridedaily_author2)) {
        $upload_baseurl = $upload_dir['baseurl'] . '/' . $tridedaily_author2;
    } else {
        $upload_baseurl = "";
    }
    $htmlaudio2 = '<p>';
    $htmlaudio2 .= 'Audio 2';
    $htmlaudio2 .= '</p>';
    $htmlaudio2 .= '<input id="upload_file" type="hidden" size="36" name="upload_file" value="" />';
    $htmlaudio2 .= '<input id="upload_file_audio2" readonly type="text" size="90%" name="upload_file_audio2" value = "' . $upload_baseurl . '"  />';
    $htmlaudio2 .='<input type="button" onclick = upload_meta_image("upload-Audio2-file","upload_file_audio2") id="upload_file_button_audio2" value="Upload Audio 2" />';
    echo $htmlaudio2 . "<br>";
}
...
function save_post_meta_box_data($post_id) {
    ...
    if ($home_featured_post == "1") {
        if (!metadata_exists('post', $post_id, "menu_order_marquee")) {
            update_post_meta($post_id, "menu_order_marquee", "");
        }
    } else {
        delete_post_meta($post_id, "menu_order_marquee", "");
    }
    ...
    if (isset($_POST['upload_file_audio1'])) {
        $upload_dir = wp_upload_dir();
        $upload_baseurl = $upload_dir['baseurl'];
        $background_file_url = $_POST['upload_file_audio1'];
        $file_array = explode($upload_baseurl, $background_file_url);
        $fileaudio1 = substr($file_array[1], 1);
        update_post_meta($post_id, 'trikdedaily_meta_audio1', $fileaudio1);
    }

    if (isset($_POST['upload_file_audio2'])) {
        $upload_dir = wp_upload_dir();
        $upload_baseurl = $upload_dir['baseurl'];
        $background_file_url = $_POST['upload_file_audio2'];
        $file_array = explode($upload_baseurl, $background_file_url);
        $fileaudio2 = substr($file_array[1], 1);
        update_post_meta($post_id, 'trikdedaily_meta_audio2', $fileaudio2);
    }
    ...
    if (isset($_POST['featured_image_as_base_hero_template_featured_image'])) {
        update_post_meta($post_id, FEATURED_IMAGE_AS_BASE_HERO_TEMPLATE_FEATURED_IMAGE, 1);
    } else {
        update_post_meta($post_id, FEATURED_IMAGE_AS_BASE_HERO_TEMPLATE_FEATURED_IMAGE, 0);
    }
}
...
/* function srcset_post_thumbnail($post_id, $size = 'image', $class = '', $srcset_sizes, $template_type = "hero", $srcArr = array('1000', '500', '400')) 
  {
  ...
  $image_type_array = array("thumbnail", "medium", "large", "post-thumbnail", "image-size-800",
  "image-size-500", "image-size-560", "image-size-400", "image-size-280", "guest-author-32",
  "guest-author-64", "guest-author-96", "guest-author-128");

  if($image)
  {
  if(in_array($size, $image_type_array))
  {
  $attachment_img_url = wp_get_attachment_image_src($attachment_id, $size);
  $image_source = $attachment_img_url[0];
  }
  else
  {
  $image_source = $upload_dir['baseurl']. '/' .$image['file'];
  }
  }

  $img_srcset = get_image_srcset($attachment_id, 'large', '', $srcArr);
  }
  else
  {
  $image_source = get_template_directory_uri() . '/images/image_blank_post.gif';
  $img_srcset = "";
  $img_alt = "";
  }

  return "<img src='".esc_attr( $image_source )."' srcset='".$img_srcset."'  sizes='".$srcset_sizes."' alt='".$img_alt."'>";
  } */
...
function most_viewed_dharma_talks() {
    //function to set post views of dharmatalks in dtalks page
...
add_action('wp_ajax_nopriv_most_viewed_dharma_talks', 'most_viewed_dharma_talks');
add_action('wp_ajax_most_viewed_dharma_talks', 'most_viewed_dharma_talks');

function setpopularpostviews($postID) {
    /* Declare it at the top of single pages so that it will automatically set the post as popular */
    global $wpdb;
    $table = "" . $wpdb->prefix . "postview";
    $ID = $postID;
    $Ipv4 = $_SERVER['REMOTE_ADDR'];
    $Date = date("Y-m-d h:i:s");
    $wpdb->insert($table, array('post_id' => $ID, 'ipv4_hit' => $Ipv4, 'date' => $Date)
    );
    //echo $wpdb->last_query;
}

function getMostPopular($postType) {
    global $wpdb;
    if (!empty($postType)) {
        /* get the most popular for magazine, single pages trikedaily and others according to post types . */

        $myrows = $wpdb->get_results("select post_id, count(*) as counter from " . $wpdb->prefix . "postview  "
                . "INNER JOIN " . $wpdb->prefix . "posts on " . $wpdb->prefix . "postview.post_id  = " . $wpdb->prefix . "posts.ID  and  " . $wpdb->prefix . "posts.post_type = '" . $postType . "' "
                . "where date_format(date,'%Y-%m-%d')"
                . " BETWEEN date_sub(date_format(NOW(),'%Y-%m-%d'), INTERVAL 60 DAY) AND NOW()"
                . " group by " . $wpdb->prefix . "postview.post_id order by counter DESC ");
        return $myrows;
    } else {

        $myrows = $wpdb->get_results("select pview.post_id, count(*) as counter from " . $wpdb->prefix . "postview  as pview"
                . " inner join " . $wpdb->prefix . "posts as ptable on pview.post_id = ptable.ID where ptable.post_status='publish' and date_format(pview.date,'%Y-%m-%d')"
                . " BETWEEN date_sub(date_format(NOW(),'%Y-%m-%d'), INTERVAL 14 DAY) AND NOW()"
                . " group by pview.post_id order by counter DESC ");
        return $myrows;
    }
}
...
function loadAdvOptions() {
    ...
    $radiooption['filmfestival_ad1_'] = $radiooption['topics_ad1_'];
    $radiooption['filmfestival_ad2_'] = $radiooption['topics_ad1_'];
    $radiooption['filmfestival_archive_ad_'] = $radiooption['topics_ad1_'];
    ...
    $radiooption['pages_'] = $radiooption['topics_ad1_'];
    $radiooption['search_'] = $radiooption['topics_ad1_'];
    $radiooption['square_ad_'] = array(
        array("key" => "home_page_ad_type", "value" => "google_dfp", "label" => "Paid Ad / House ad"));
...
function loadmoreajax() {
    ...
    while ($eq_query->have_posts()): $eq_query->the_post();
        ...
        <?php } ?>
                    <?php
                    //if($post_type == MAGAZINE_POST_TYPE ||  $post_type == EBOOKS_POST_TYPE)
                    //{
                    $imsgSet = 'image-size-500';
                    $attr_size = "(min-width: 37.5em) 25vw, 100vw";
                    $source_width = array(1000, 500);
                    //}else{
                    //  $imsgSet =  'image-size-500';
                    //$attr_size = "(min-width: 37.5em) 25vw, 100vw";
                    //$source_width = array(1000, 500);
                    //}
                    echo get_image_with_srcset(array("post_id" => get_the_ID(), "image_type" => "thumbnail", "source_width" => $source_width, "size" => $imsgSet, "attr_sizes" => $attr_size));
                    //echo srcset_post_thumbnail(get_the_ID(), $imsgSet, '', '(min-width: 37.5em) 25vw, 100vw');
                    ?>
                </a>
            </figure>
            <section class = 'titles'>
                ...
                <p><?php echo excerpt(35); ?></p>
                    <?php if ($post_type != EBOOKS_POST_TYPE) { // as we dont need author and time for ebooks
                        ?>
                    <address><?php echo getAuthorDetail(get_the_ID(), 1, 0); ?></address><time><?php echo ($post_type == MAGAZINE_POST_TYPE) ? $issues_names_without_urls : (($post_type == "trikedaily") ? get_the_date('M d, Y') : get_the_date('M Y')); ?></time>
        <?php } ?>
            </section>
        </article>
                <?php
...
function archiveloadmore() {
    ...
    while ($queryfirst->have_posts()) : $queryfirst->the_post();
        ...
        if ($posttype == MAGAZINE_POST_TYPE || $posttype == EBOOKS_POST_TYPE) {
            $imsgSet = 'image-size-400';
            $srcArr = array('800', '400');
            $attr_width = "(min-width: 60em) 33vw,(min-width: 37.5em) 50vw,75vw";
        } else if ($posttype == DHARMATALKS_POST_TYPE) {
            $imsgSet = 'image-size-500';
            $srcArr = array('2000', '1000', '500');
            $attr_width = "(min-width: 50em) 33vw,100vw";
        } else if ($posttype == FILM_CLUB_POST_TYPE) {
            $imsgSet = 'image';
            $srcArr = array('2000', '1000', '500');
            $attr_width = "(min-width: 60em) 33vw,(min-width: 37.5em) 50vw,100vw";
        } else {
            $imsgSet = 'image-size-400';
            $srcArr = array('800', '400');
            $attr_width = "(min-width: 60em) 33vw,(min-width: 37.5em) 50vw,75vw";
        ...
        $childitemStr .= get_image_with_srcset(array("post_id" => get_the_ID(), "image_type" => "thumbnail", "source_width" => $srcArr, "size" => $imsgSet, "attr_sizes" => $attr_width));
        ...
        if ($posttype != "ebooks") {
            if ($posttype == FILM_CLUB_POST_TYPE) {
                //as author needs to be link with article
                $childitemStr .="<address><a href= '" . get_the_permalink() . "'> " . strip_tags(getAuthorDetail(get_the_ID(), 0, 0)) . "</a></address>";
            } else {
    ...
    if ($eq_total_posts > "0") {
        ...
        $pagingStr .='<div class="loader-more">';
        $pagingStr .='<div id="loadingimage" style="position:relative"></div>';
...
function MagazineLoadMore() {
    ...
    foreach ($topicsfinal as $topics) {
        ...
        if (!empty($oldyear)) { //condition to check whene browsing year is there ........
            if ($year > $oldyear) {
                continue;
            }
        }
        ...
        if (in_array($topics->term_id, $topics_termid)) {
            ...
            $pageHtml .= get_image_with_srcset(array("image_type" => "magazine_issue_cover_image", "term_id" => $t_id, "source_width" => array(800, 400), "size" => "image-size-400", "attr_sizes" => '(min-width: 60em) 33vw,(min-width: 37.5em) 50vw,75vw'));
            //$pageHtml .= getImagewithSrcset($magazine_issue_cover_image, 'image-size-400', '', '(min-width: 60em) 33vw,(min-width: 37.5em) 50vw,75vw', array(800, 400));
            $pageHtml .="</figure>";
    ...
    if ($count == "12") {
        $pagingHtml .= '<div class="loader-more">';
        ...
        $pagingHtml .="<input type='hidden' name='oldyear' id='oldyear' value='" . $oldyear . "' />";
...
function LoadMoreAuthor() {
    ...
    while ($eq_query->have_posts()): $eq_query->the_post();
        ...
        echo get_image_with_srcset(array("post_id" => get_the_ID(), "image_type" => "thumbnail", "source_width" => array(1000, 500), "size" => "image-size-500", "attr_sizes" => "(min-width: 37.5em) 25vw, 100vw"));
        //echo srcset_post_thumbnail(get_the_ID(), 'image', '', '(min-width: 50em) 33vw,100vw'); 
        ?>
                </a>
            </figure>
            <section class = 'titles'>
...
function custom_post_author_archive($query) {
    if ($query->is_author() && $query->is_main_query()) {
        ...
        $query->set('post_type', array('post', MAGAZINE_POST_TYPE, DHARMATALKS_POST_TYPE, FILM_CLUB_POST_TYPE, EBOOKS_POST_TYPE, FILM_FESTIVAL_POST_TYPE));
    } else if (is_post_type_archive(EBOOKS_POST_TYPE) && $query->is_main_query()) {
        $query->set("posts_per_page", 12);
    } else if (is_post_type_archive(FILM_FESTIVAL_POST_TYPE) && $query->is_main_query()) {
        $query->set("posts_per_page", -1);
        $query->set('meta_key', 'menu_order_filmfestival');
        $query->set('orderby', 'meta_value_num');
        $query->set('order', 'ASC');
    } else if ((is_year() || is_month() || is_day()) && $query->is_main_query()) {
        $query->set("posts_per_page", -1);
    }
}
    ...
    if (!empty($Department)) {
        foreach ($Department as $department) {
            if ($department->parent != 0) {
                $dept[] = '<a href="' . get_term_link($department) . '">' . $department->name . '</a>';
            }
        }
        echo implode(", ", $dept);
    }
...
function get_filmfest_department($postId, $department) {
    ...
    $dept = array();
    if (!empty($Department)) {
        foreach ($Department as $filmfestdepartment) {
            $dept[] = $filmfestdepartment->name;
        }
    }
    echo implode(", ", $dept);
}
...
function get_trikedaily_department($postId) {
    ...
    if (!empty($Category)) {
        foreach ($Category as $category) {
            ...
            $dept[] = '<a href="' . $categoryLink . '">' . $category->name . '</a>';
        }
        echo implode(", ", $dept);
    }
}
...
function show_latest_ebooks($number_of_posts = 1) {
    ...
    ?>
        ...
        function add_paywall_meta_box($postType) {
            $types = array(MAGAZINE_POST_TYPE, DHARMATALKS_POST_TYPE, FILM_CLUB_POST_TYPE, EBOOKS_POST_TYPE, FILM_FESTIVAL_POST_TYPE);
        ...
        function tricycle_paywall_meta_box($post) {
            ...
            if ($post->post_status == "auto-draft") {
                ?>
        <label><input type="checkbox" value="1" checked="checked" name="behind_the_paywall"  />Behind the Paywall</label>
            <?php } else { ?>
...
function get_tool_tip_sharethis($title, $url, $post_ID) {
    ?>
    <div class = 'tools_top'>
        <ul>
            <li class="st_facebook_custom" st_title='<?php echo $title; ?>' st_url='<?php echo $url; ?>'><a onClick="ga('send', 'event', {eventCategory: 'share', eventAction: 'facebook', eventLabel: 'article-top'});" class='icon icon-tools_facebook' href='javascript:void(0)' data-grunticon-embed></a></li>
            <li class="st_twitter_custom" st_title='<?php echo $title; ?>' st_url='<?php echo $url; ?>'><a onClick="ga('send', 'event', {eventCategory: 'share', eventAction: 'twitter', eventLabel: 'article-top'});" class='icon icon-tools_twitter' href='javascript:void(0)' data-grunticon-embed></a></li>
            <li class="st_email_custom" st_title='<?php echo $title; ?>' st_url='<?php echo $url; ?>'><a onClick="ga('send', 'event', {eventCategory: 'share', eventAction: 'email', eventLabel: 'article-top'});" class = 'icon icon-tools_email' href = '#' data-grunticon-embed></a></li>
            <li><a class ='icon icon-tools_print' onclick="window.open('<?php echo add_query_arg("pid", $post_ID, get_permalink(get_page_by_path("print"))); ?>', '', 'width=900, height=500, scrollbars=yes')"  data-grunticon-embed></a></li>
        </ul>
    </div>
    <?php
}
...
function get_tool_tip_sharethis_wrap($title, $url, $post_ID) {
    ?>
    <div class = 'tools-wrap_bottom'>
        <ul>
            <li class="st_facebook_custom" st_title='<?php echo $title; ?>' st_url='<?php echo $url; ?>'><a onClick="ga('send', 'event', {eventCategory: 'share', eventAction: 'facebook', eventLabel: 'article-bottom'});" class='icon icon-tools_facebook' href='javascript:void(0)' data-grunticon-embed></a></li>
            <li class="st_twitter_custom" st_title='<?php echo $title; ?>' st_url='<?php echo $url; ?>'><a onClick="ga('send', 'event', {eventCategory: 'share', eventAction: 'twitter', eventLabel: 'article-bottom'});" class='icon icon-tools_twitter' href='javascript:void(0)' data-grunticon-embed></a></li>
            <li class="st_email_custom" st_title='<?php echo $title; ?>' st_url='<?php echo $url; ?>'><a onClick="ga('send', 'event', {eventCategory: 'share', eventAction: 'email', eventLabel: 'article-bottom'});" class = 'icon icon-tools_email' href = '#' data-grunticon-embed></a></li>
            <li><a class ='icon icon-tools_print' onclick="window.open('<?php echo add_query_arg("pid", $post_ID, get_permalink(get_page_by_path("print"))); ?>', '', 'width=900, height=500, scrollbars=yes')"  data-grunticon-embed></a></li>
        </ul>
    </div>
    <?php
...
function get_paywall_login($pageclass, $title) {
    ...
    if (!empty($fetch_paywall_module)) {
        ...
        foreach ($fetched_result as $key => $value) {
            if ($key == $pageclass) {
                $image = $value["moduleicon"];
                $title = $value["moduletitle"];
                $text = $value["moduletext"];
                break;
            }
        }
    }
    ...
    <?php if (!empty($image)) { ?>
                <span class='icon'><img src="<?php echo $base_url . $image; ?>"/></span>
    <?php } else { ?>
                <span class='icon icon-paywall_print'></span>
    <?php } ?>
            <h1 class='p'><?php echo $title; ?></h1>
            <p><?php echo $text; ?></p>
            <a onClick="ga('send', 'event', {eventCategory: 'subscribe', eventAction: 'click', eventLabel: 'paywall'});" class='button' href='https://subscribe.tricycle.com/TRI/?f=join' target='_blank'>Subscribe Now</a>
            <p>Already a subscriber? Log in.</p>
            <form class='login-form' name='login' id="login_paywall" ref="<?php echo get_template_directory_uri(); ?>">
                <input class='email' placeholder='Email' type='email' name="l_email" id="l__paywall_email" required>
                <input class='pass' placeholder='Password' type='password' name="l_pass" id="l_paywall_pass" required>
                <button onClick="ga('send', 'event', {eventCategory: 'login', eventAction: 'click', eventLabel: 'paywall'});" class='button' type='submit' id="login-paywall-button">Login</button>
...
function get_paywall_content($content) {
    ...
    foreach ($topcontent as $top_three_paragraph) { //echo $top_three_paragraph."========";  
        ...
        if ($i == 5) {
            $sixthElement = $top_three_paragraph;
            break;
        }
        $i++;
    }
    if ($sixthElement) {
        $contentmew = explode($sixthElement, $content);
    } else {
        if ($top_paragraph) {
            $contentmew = explode($top_paragraph, $content);
        }
    }
    $top_paragraph = $contentmew[0] . $sixthElement;
...
function get_newsletter_signup_module() {
    ...
    $enewsimage = get_option("settings-enewsimage");
    $enewsheadline = get_option("settings-enewsheadline");
    $enewstext = get_option("settings-enewstext");
    if ($enewsimage) {
        $enews_image = $upload_url . '/' . $enewsimage;
    }
    ?>
    <div class="CTA-module_enews">
        <figure>
            <img alt="Temple" sizes="(min-width: 37.5em) 33vw,
                 (min-width: 25em) 75vw,
                 100vw" srcset="<?php echo $enews_image; ?> 400w,
    <?php echo $enews_image; ?> 200w" src="<?php echo $enews_image; ?>">
        </figure>
        <h1 class="p"><?php echo $enewsheadline; ?></h1>
        <p><?php echo $enewstext; ?></p>
        <form id="CTA-enews">
            <input type="email" placeholder="Your email here" id="action-email" class="email">
            <button onClick="ga('send', 'event', {eventCategory: 'enews', eventAction: 'click', eventLabel: 'CTA-module'});" id="CTA-modal-trigger">Sign Up</button>
        </form>
    </div>
    <?php
}
...
function get_subscribe_module() {
    $upload_dir = wp_upload_dir();
    $upload_url = $upload_dir['baseurl'];
    $subscribeimage = get_option("settings-subscribeimage");
    $subscribeheadline = get_option("settings-subscribeheadline");
    $subscribetext = get_option("settings-subscribetext");
    if ($subscribeimage) {
        $subscriber_image = $upload_url . '/' . $subscribeimage;
    }
    ?>
    <div class="CTA-module_subscribe">
        <figure>
            <img alt="Temple" sizes="(min-width: 37.5em) 33vw,
                 (min-width: 25em) 75vw,
                 100vw" srcset="<?php echo $subscriber_image ?> 400w,
    <?php echo $subscriber_image; ?> 200w" src="<?php echo $subscriber_image; ?>">
        </figure>
        <h1 class="p"><?php echo $subscribeheadline; ?></h1>
        <p><?php echo $subscribetext; ?></p>
        <a onClick="ga('send', 'event', {eventCategory: 'subscribe', eventAction: 'click', eventLabel: 'CTA-module'});" href='https://subscribe.tricycle.com/TRI/?f=join' target="_blank" class="button">Subscribe now</a>
    </div>
    <?php
}
...
function get_dailydharma() {
            ...
            ?>
                ...
                <div class='social-wrap'>
                    <ul class='dailydharma-social'>
                        <li class="st_facebook_custom" st_title='<?php the_content(); ?>' st_url='<?php
            if (empty($dailydharma_url)) {
                the_permalink();
            } else {
                echo $dailydharma_url;
            }
            ?>'><a onClick="ga('send', 'event', {eventCategory: 'share', eventAction: 'facebook', eventLabel: 'daily-dharma'});" class='icon icon-share_facebook' href='javascript:void(0)' data-grunticon-embed></a></li>
                        <li class="st_twitter_custom" st_title='<?php the_content(); ?>' st_url='<?php
                    if (empty($dailydharma_url)) {
                        the_permalink();
                    } else {
                        echo $dailydharma_url;
                    }
                    ?>'><a onClick="ga('send', 'event', {eventCategory: 'share', eventAction: 'twitter', eventLabel: 'daily-dharma'});" class='icon icon-share_twitter' href='javascript:void(0)' data-grunticon-embed></a></li>
                        <li class="st_email_custom" st_title='<?php the_content(); ?>' st_url='<?php
                    if (empty($dailydharma_url)) {
                        the_permalink();
                    } else {
                        echo $dailydharma_url;
                    }
                    ?>'><a onClick="ga('send', 'event', {eventCategory: 'share', eventAction: 'email', eventLabel: 'daily-dharma'});" class='icon icon-share_email' href='javascript:void(0)' data-grunticon-embed></a></li>
                    </ul>
                </div>
            <?php
        endwhile;
    ...
    ?>        
        <form id='dailydharma-enews'>
            <input class='email' id='dailydharma-email' placeholder='Your email here' type='text'>
...
// end of daily dhamra function 
                    ...
                    if (class_exists("TricycleAdManagement")) {
                        ...
                        if ($ads) {
                            ...
                            if ($ad_type == "display_nothing") {
                                //echo '<div style="margin: 0 0 0 0"></div>';
                            } else {
                                if ($adlocation != "square_ad_" && $adlocation != "header_ad_") {
                                    ?>
                    <div class="rule"></div>
                <?php } ?>
                <?php
    ...
    /* function srcset_post_base_hero_template($imagePath, $size = 'image', $class = '', $srcset_sizes, $template_type = "hero")
      {
      $attachment_id = get_image_attachment_id($imagePath);
      $img_alt = trim( strip_tags( get_post_meta( $attachment_id, '_wp_attachment_image_alt', true ) ) );
      ...
      $img_src = get_template_directory_uri() . '/images/image_blank_post.gif';
      }

      $img_srcset = get_image_srcset( $attachment_id, 'large' );
      return $returnImage = "<img src='".esc_attr( $img_src )."' srcset='".esc_attr( $img_srcset )."'  sizes='".$srcset_sizes."' alt='".$img_alt."'>";
      } */

    function getpaywall_popup($classname, $title, $cntr = 0) {
        $image = '';
        $title = '';
        $text = '';
        $fetch_paywall_module = get_option("paywall_module_copy");
    ...
    <?php } ?>
                        ...
                        <a  onClick="ga('send', 'event', {eventCategory: 'subscribe', eventAction: 'click', eventLabel: 'paywall'});"  class='button' href='https://subscribe.tricycle.com/TRI/?f=join' target='_blank'>Subscribe Now</a>
                        <p>Already a subscriber? Log in.</p>
                        <form onsubmit="return false" class='login-form' name='login' id="login_paywall_popup<?php echo $cntr; ?>" ref="<?php echo get_template_directory_uri(); ?>">
                            <input class='email l__paywall_email' placeholder='Email' type='email' name="l_email" id="l__paywall_email" required>
                            <input class='pass l_paywall_pass' placeholder='Password' type='password' name="l_pass" id="l_paywall_pass" required>
                            <button onClick="ga('send', 'event', {eventCategory: 'login', eventAction: 'click', eventLabel: 'paywall'});" class='button login-paywall-button' type='submit' id="login-paywall-popup-button">Login</button>
                            <a class='link' target="_blank" href='https://subscribe.tricycle.com/TRI/?f=custreg'>Need help with email or password?</a>
                        </form>
                    </div>
                </div>
                <!-- end article paywall -->
                    ...
                    function is_latest_post($post_id, $query_args = array(), $post_type) {
                    ...
                    /* To remove top space when logged in */
                    ...
                    function get_image_attachment_id($url) {
                        global $wpdb;
                    ...
                    function add_column_to_media_library($columns) {
                        $columns['colURL'] = __('URL');
                        return $columns;
                    }

                    add_filter('manage_media_columns', 'add_column_to_media_library');

                    // Register the column as sortable & sort by name
                    function filename_column_sortable($columns) {
                        $columns['colURL'] = "URL";
                        return $columns;
                    }

                    function media_library_column_row($columnName, $columnID) {
                        if ($columnName == 'colURL') {
                            $attachment_link = wp_get_attachment_url($columnID);
                            echo $attachment_link;
                        }
                    }

                    add_filter('manage_media_custom_column', 'media_library_column_row', 10, 2);
                    add_filter('manage_upload_sortable_columns', 'filename_column_sortable');

                    function get_image_with_srcset($args = array('post_id' => NULL, 'size' => 'full', 'image_type' => 'thumbnail', 'term_id' => null, 'attr_sizes' => null, 'source_width' => array('1000', '500', '400'))) {
                        global $wpdb;
                        $upload_dir = wp_upload_dir();

                        $image_type_array = array("thumbnail", "medium", "large", "post-thumbnail", "image-size-800",
                            "image-size-500", "image-size-560", "image-size-2001", "image-size-1001", "image-size-501", "image-size-400", "image-size-280", "guest-author-32",
                            "guest-author-64", "guest-author-96", "guest-author-128", "full");
                        ...
                        if ($args['image_type'] == "magazine_issue_cover_image") {
                            $t_id = $args['term_id'];
                            $term_meta = get_option("taxonomy_term_$t_id");
                            $imageurl = $term_meta['cover_image'];
                            if ($imageurl) {
                                $attachment_id = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_type='attachment' AND guid LIKE '%" . $imageurl . "%'");

                                if ($attachment_id) {
                                    if (in_array($args['size'], $image_type_array)) {
                                        $attachment_img_url = wp_get_attachment_image_src($attachment_id, $args['size']);
                                        $image_source = $attachment_img_url[0];
                                    } else {
                                        $attachment_img_url = wp_get_attachment_image_src($attachment_id, "full");
                                        $image_source = $attachment_img_url[0];
                                    }

                                    $img_srcset = get_srcset($attachment_id, $args['source_width']);
                                } else {
                                    $image_source = get_template_directory_uri() . '/images/img_blank.gif';
                                    $img_srcset = "";
                                }
                            } else {
                                $image_source = get_template_directory_uri() . '/images/img_blank.gif';
                        ...
                        } else if ($args['image_type'] == "thumbnail") {
                        ...
                        } else if ($args['image_type'] == "hero_template_image" || $args['image_type'] == "base_template_image") {
                            $base_hero_template_featured_image = get_post_meta($args['post_id'], BASE_HERO_TEMPLATE_FEATURED_IMAGE, TRUE);
                            $is_featured_image_set_as_base_and_hero_template_featured_image = get_post_meta($args['post_id'], FEATURED_IMAGE_AS_BASE_HERO_TEMPLATE_FEATURED_IMAGE, TRUE);
                            if ($base_hero_template_featured_image) {
                                $attachment_id = $base_hero_template_featured_image['attachment_id'];
                                if ($attachment_id) {
                                    if (in_array($args['size'], $image_type_array)) {
                                        $attachment_img_url = wp_get_attachment_image_src($attachment_id, $args['size']);
                                        $image_source = $attachment_img_url[0];
                            ...
                            } else if ($is_featured_image_set_as_base_and_hero_template_featured_image == 1) {
                                $attachment_id = get_post_thumbnail_id($args['post_id']);
                                if ($attachment_id) {
                                    $attachment_img_url = wp_get_attachment_image_src($attachment_id, $args['size']);
                                    $image_source = $attachment_img_url[0];
                        ...
                        if ($image_source) {
                            return "<img src='" . esc_attr($image_source) . "' srcset='" . $img_srcset . "'  sizes='" . $args['attr_sizes'] . "' alt='" . $img_alt . "'>";
                        } else {
                            return;
                        }
                    }

                    function get_srcset($attachment_id, $source_width) {
                        $image = wp_get_attachment_metadata($attachment_id);
                        $max_source_width = max($source_width);
                        $full_source_set = 0;
                        if ($image) {
                            $upload_dir = wp_upload_dir();
                            ...
                            foreach ($image['sizes'] as $size) {
                                if (in_array($size['width'], $source_width)) {
                                    $sizes[$size['width']] = $upload_url . $size['file'] . ' ' . $size['width'] . 'w';
                                }
                                /* else
                                  {
                                  if($image['width'] >= 500 && $max_source_width >= $image['width'] && !$full_source_set)
                                  {
                                  $sizes[$image['width']] = $upload_dir['baseurl'] . '/' . $image['file'] . ' ' . $image['width'] . 'w';
                                  $full_source_set = 1;
                                  }
                                  } */
                            }
                    ...
                    /*
                     * Function to add users tab for editor in wordpress
                     */

                    function editors_user_role() {
                        $edit_editor = get_role('editor'); // Get the user role
                        if (is_object($edit_editor) && method_exists($edit_editor, 'add_cap')) {
                    ...
                    add_action('init', 'editors_user_role');

                    /**
                     * This function removes the height and width attribute from the post content.
                     * @param string $html
                     * @return string
                     */
                    ...
                    /**
                     * This filter is used to call "remove_image_dimensions" function.
                     */
                    add_filter('the_content', 'remove_image_dimensions', 10);

                    /**
                     * This function is used to filter image src set.
                     * @param array $sources
                     * @return array
                     */
                    function filter_image_srcset($sources) {
                        if (is_singular()) {
                            foreach ($sources as $key => $source) {
                                if (in_array($key, array(500, 1000, 2000))) {
                    ...
                    /**
                     * This filter is used to call "filter_image_srcset" function.
                     */
                    add_filter('wp_calculate_image_srcset', 'filter_image_srcset');

                    /*
                     * Cron for film lcub shopoots when someone scheduled the post 
                     */

                    function callcronfilmclub() {
                        $path = get_home_path() . '/cron_filmclub.php';
                        include_once $path;
                    }

                    add_action('future_to_publish', 'callcronfilmclub');

                    /**
                     * This function is used to set "size" property for "srcset".
                     * @param string $sizes
                     * @param type $size
                     * @return string
                     */
                    function adjust_image_sizes_attr($sizes, $size) {
                        if (is_singular()) {
                            $sizes = '(min-width: 60em) 66vw, 100vw';
                        }
                        return $sizes;
                    }

                    /**
                     * This filter is used to call the function "adjust_image_sizes_attr"
                     */
                    add_filter('wp_calculate_image_sizes', 'adjust_image_sizes_attr', 10, 2);

                    /**
                     * This function is used to add image size 2000 in the srcset.
                     * @param type $max_width
                     * @param type $size_array
                     * @return int
                     */
                    ...
                    /**
                     * This function is used to call the function "custom_max_srcset_image_width"
                     */
                    add_filter('max_srcset_image_width', 'custom_max_srcset_image_width', 10, 2);

                    /**
                     * This function is used to add custom size in the dropdown in "Display Settings" while inserting the image in the body.
                     * @param type $sizes
                     * @return type
                     */
                    function add_custom_image_sizes_in_display_setting($sizes) {
                        $addsizes = array(
                            "image-size-500" => __("Custom Size")
                        );
                    ...
                    /**
                     * This function is used to call the function "add_custom_image_sizes_in_display_setting"
                     */
                    add_filter('image_size_names_choose', 'add_custom_image_sizes_in_display_setting');

                    /**
                     * This function is used to wrap the "img" tag with "figure" tag in place of "p" tag while rendering the post content.
                    ...
                    /**
                     * This filter is used to call the function "img_unautop".
                     */
                    add_filter('the_content', 'img_unautop', 30);

                    /**
                     * This function is used to change the permalink structure (remove 'with front' -> trikedaily) of author, category and tag.
                     * @global type $wp_rewrite
                     */
                    function change_permalinks() {
                        global $wp_rewrite; //pr($wp_rewrite);
                    ...
                    /**
                     * This action is used to call the function "change_permalinks".
                     */
                    add_action('init', 'change_permalinks');


                    add_action('generate_rewrite_rules', 'add_rewrite_tules');

                    function add_rewrite_tules($wp_rewrite) {
                        $newrules = array();
                        $newrules['feature/?$'] = 'index.php?post_type=magazine';
                        //$newrules['work/page/?([0-9]{1,})/?$'] = 'index.php?post_type=work&paged=$matches[1]';
                        //$newrules['work/clients/?([0-9]{1,})/?$'] = 'index.php?clients=?$])';
                        $wp_rewrite->rules = $newrules + $wp_rewrite->rules;
                    }

                    //This function is used to flush the rewrite rules.
                    flush_rewrite_rules();
                    add_rewrite_rule('feature/?$', 'index.php?post_type=magazine', 'top');
                    /*
                     * Function to get header ads different on every page
                     */
                    ...
                    function get_header_ad($post_type) {
                        if ($post_type == "front_page") {
                            ...
                            $pathads['path_ad_mob'] = "/52618277/emptyads";
                            $pathads['path_ad_desk'] = "/52618277/emptyads";
                        }
                        return $pathads;
                    }

                    /*
                     * function to read cambay cwrecid and auto log in user
                     */

                    function cwreidcheck($cwrecid) {
                        $message = array();
                        $url = "https://www.cambeywest.com/api_rest/api_rest.svc/GetSubscriberData_ByCWRecId?pub_acronym=TRI&auth_user=mr39d6pl&auth_pass=44gnjn2aq";
                        $url .= "&cwrecid=" . $cwrecid;
                        $ch = curl_init();
                        ...
                        if ($result1) {
                            foreach ($result1 as $string) {
                                $xml = simplexml_load_string($string, 'SimpleXMLElement', LIBXML_NOCDATA);
                            }
                            if (strtolower((string) $xml->SubscriberData->result) == "true" && trim((string) $xml->SubscriberData->frienderrormsg) == "") {
                                $login = 1;
                                $first_name = (string) $xml->SubscriberData->f_name;
                                $last_name = (string) $xml->SubscriberData->l_name;
                                $Cambey_Login = $first_name . " " . $last_name;
                                $a_no = (int) $xml->SubscriberData->acctno;
                                //return $message = array("name"=>$Cambey_Login, "acount_no" =>$a_no, "message"=>"success");
                                $cookie_time = time() + (86400 * 365);
                                $cl = setcookie("Cambey_Login", $Cambey_Login, $cookie_time, "/", "tricycle.org");
                                $clacc = setcookie("Cambey_Account", base64_encode($a_no), $cookie_time, "/", "tricycle.org");
                                if ($cl == "1" && $clacc = "1") {
                                    return $message = array("cambay_login" => $Cambey_Login, "cambay_account" => base64_encode($a_no));
                                }
                            }
                        }
                    }

                    function include_hidden_tag() {
                        if (is_single()) {
                            ?>
        <input type="hidden" name="interstitial-ad-tag" value="1" id="interstitial-ad-tag">
        <?php
    }
}

add_action('wp_footer', 'include_hidden_tag');
